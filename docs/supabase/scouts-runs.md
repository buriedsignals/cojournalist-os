...
| `excluded_domains` | TEXT[] | beat |
| `priority_sources` | TEXT[] | beat (added 00007); required for beat scout |
...
| `location` | JSONB | `GeocodedLocation`, beat |
...
| `articles_count` | INT | How many new canonical units this run created |
| `merged_existing_count` | INT DEFAULT 0 | How many occurrences attached to existing canonical units |
...
Source of truth for scraped content. Used by web/civic/beat extraction and verification.
...
7. Else: store `raw_captures` row. If `scout.criteria` set: `geminiExtract(EXTRACTION_SCHEMA)` → within-run paraphrase filter → `upsert_canonical_unit(...)` (same-scout exact → cross-scout exact → semantic canonical match).
8. Firecrawl/Gemini calls are client-time-bounded; primary page scrape and Phase B subpage-follow both run under explicit per-call caps so a single external stall cannot leave `scout_runs` pinned in `running`.
9. On throw: mark run error, `increment_scout_failures`, return error.

### `scout-beat-execute`
1. Service-key auth.
2. Load scout, require `priority_sources` + `criteria`.
3. **`decrementOrThrow` for `beat`** (7 credits).
...
7. Within-run paraphrase filter + `upsert_canonical_unit(...)`, mark run, reset/increment failures.
...
5. Apify actor input is posted as the raw actor input object, not wrapped under `{"input": ...}`. This matches the synchronous preview path and avoids empty async runs with successful queue rows.
6. Later: `apify-callback` receives the run result, processes posts via `apify-reconcile`.
...
5. **Canonical units survive scout deletion.** `information_units.scout_id`, `promises.scout_id`, and `unit_occurrences.scout_id` are nullable origin/provenance pointers; deleting a scout removes scheduled/runtime state, not verified knowledge.
6. **Social scouts use the canonical dedup path.** Same-social reruns dedup like every other scout. Exact matches can merge with non-social scouts, but semantic matching does not cross the `social` / `non-social` boundary.

## Live benchmark

Reusable end-to-end dedup smoke:
...
The benchmark creates a temporary user, seeds credits, runs web + beat + civic + social, checks canonical-unit convergence/provenance, runs a synthetic probe that verifies the `social` / `non-social` semantic boundary, then deletes the temp user.
...
- `docs/features/web-scouts.md`, `beat.md`, `social.md`, `civic.md` — feature-level behaviour
