...
EXCLUDE: Breaking news already covered in the news section, press releases without analysis.`;
	...
	async function handleSearch() {
		...
		try {
			...
			console.error('[SmartScoutView] Search error:', error);
			stopProgress(false);
		}
	}
			...
			>
				...
				{#if investigationType === 'location' && sourceMode === 'reliable'}
					<div class="field-group">
						<label for="smart-criteria" class="field-label">
							{m.smartScout_criteriaLabel()}
							<span class="field-subtitle">{m.smartScout_criteriaOptionalHint()}</span>
						</label>
						<CriteriaInput
							bind:value={topicInput}
							placeholder={m.webScout_criteriaPlaceholder()}
...
<style>
	...
	.prompt-toggle:hover { background: #f3f4f6; border-color: var(--color-accent); }
	...
	.prompt-textarea::placeholder { color: #9ca3af; font-size: 0.6875rem; }
	...
	.reset-button:hover { background: #fee2e2; border-color: #fca5a5; color: #dc2626; }
	...
	.query-tag { padding: 0.25rem 0.5rem; background: #f3f4f6; border-radius: var(--radius-sm); font-size: 0.6875rem; color: var(--color-text-secondary); }
</style>
