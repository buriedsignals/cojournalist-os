...
class TestRunScout:
    def test_run_scout_success(self, api_client):
        mock_svc = AsyncMock()
        mock_svc.get_scout.return_value = {"name": "Runner", "scout_type": "web"}

        with patch.object(v1_module, "_get_schedule_service", return_value=mock_svc), \
             patch("app.routers.v1.ScoutRunner") as MockRunner, \
             patch("app.routers.v1.validate_credits", new_callable=AsyncMock) as mock_validate, \
             patch("app.routers.v1.decrement_credit", new_callable=AsyncMock) as mock_decrement:
            mock_validate.return_value = {"current_credits": 200, "required": 1}
            mock_decrement.return_value = True
            MockRunner.return_value.run_scout = AsyncMock(return_value={
                "scraper_status": True,
                "criteria_status": True,
            ...
            response = api_client.post("/api/v1/scouts/Runner/run")
    ...
    def test_run_pulse_scout_costs_7_credits(self, api_client):
        mock_svc = AsyncMock()
        mock_svc.get_scout.return_value = {"name": "Pulse", "scout_type": "pulse"}

        with patch.object(v1_module, "_get_schedule_service", return_value=mock_svc), \
             patch("app.routers.v1.ScoutRunner") as MockRunner, \
             patch("app.routers.v1.validate_credits", new_callable=AsyncMock) as mock_validate, \
             patch("app.routers.v1.decrement_credit", new_callable=AsyncMock) as mock_decrement:
            mock_validate.return_value = {"current_credits": 200, "required": 7}
            mock_decrement.return_value = True
            MockRunner.return_value.run_scout = AsyncMock(return_value={
                "scraper_status": True, "criteria_status": True, "summary": "News found",
            })
    ...
    def test_run_scout_insufficient_credits_returns_402(self, api_client):
        ...
        mock_svc = AsyncMock()
        mock_svc.get_scout.return_value = {"name": "Poor", "scout_type": "web"}

        async def raise_402(*args, **kwargs):
            raise HTTPException(
                status_code=402,
                detail={"error": "insufficient_credits", "message": "Not enough"},
            )
        ...
        with patch.object(v1_module, "_get_schedule_service", return_value=mock_svc), \
             patch("app.routers.v1.ScoutRunner") as MockRunner, \
             patch("app.routers.v1.validate_credits", side_effect=raise_402), \
             patch("app.routers.v1.decrement_credit", new_callable=AsyncMock) as mock_decrement:

            response = api_client.post("/api/v1/scouts/Poor/run")

        assert response.status_code == 402
        # Credits should NOT be deducted when validation fails
        mock_decrement.assert_not_called()
        # run_scout should NOT be called either
        MockRunner.return_value.run_scout.assert_not_called()

    def test_run_nonexistent_scout_returns_404(self, api_client):
        mock_svc = AsyncMock()
        mock_svc.get_scout.return_value = None

        with patch.object(v1_module, "_get_schedule_service", return_value=mock_svc):
            response = api_client.post("/api/v1/scouts/Missing/run")

        assert response.status_code == 404

    def test_run_scout_internal_error_does_not_charge_credits(self, api_client):
        """If run_scout raises, credits should not be deducted."""
        mock_svc = AsyncMock()
        mock_svc.get_scout.return_value = {"name": "Broken", "scout_type": "web"}
        ...
        with patch.object(v1_module, "_get_schedule_service", return_value=mock_svc), \
             ...
             patch("app.routers.v1.decrement_credit", new_callable=AsyncMock) as mock_decrement:
            mock_validate.return_value = {"current_credits": 200, "required": 1}
            MockRunner.return_value.run_scout = AsyncMock(side_effect=RuntimeError("connection reset"))

            response = api_client.post("/api/v1/scouts/Broken/run")

        assert response.status_code == 500
        data = response.json()["detail"]
        assert data["code"] == "INTERNAL_ERROR"
        assert "Failed to run scout" in data["error"]
        # Credits must NOT be deducted when execution fails
        mock_decrement.assert_not_called()
