"""
Pydantic schemas for Beat Scout endpoints.

PURPOSE: Request/response models for POST /beat/search (UI) and
POST /beat/execute (Lambda). Includes input validation for scope
...
USED BY: schemas/__init__.py (re-exports for the Supabase Edge Function
    side of the beat pipeline; no live FastAPI router consumer after the
    2026-04-22 cutover)
"""
import re
...
class BeatSearchRequest(BaseModel):
    """Request for UI-triggered beat search."""
...
class BeatSearchResponse(BaseModel):
    """
    Response from UI-triggered beat search.
    ...
    mode: ScoutMode = Field(ScoutMode.BEAT, description="Search mode")
...
class BeatExecuteRequest(BaseModel):
    """Request for Lambda-triggered beat scout execution."""
...
class BeatExecuteResponse(BaseModel):
    """Response from Lambda-triggered beat scout execution."""
    scraper_status: bool = Field(..., description="True if search completed successfully")
    criteria_status: bool = Field(..., description="Always True for beat (always notify)")
