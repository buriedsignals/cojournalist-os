...
export function extractCivicLinksFromHtml(
  ...
  for (const match of rawLinks) {
    ...
    if (!rawHref) continue;
    if (CIVIC_DENYLIST_PREFIXES.some((prefix) => rawHref.startsWith(prefix))) continue;
    const lowerHref = rawHref.toLowerCase();
    if (CIVIC_DENYLIST_EXTENSIONS.some((ext) => lowerHref.endsWith(ext))) continue;
...
export async function classifyCivicMeetingUrls(
  ...
  const keywordMatches = links
    .filter((link) => hasMeetingKeyword(`${link.url} ${link.anchorText}`.toLowerCase()))
    .map((link) => link.url);

  if (keywordMatches.length > 0) {
    const pdfMatches = keywordMatches.filter((url) => url.toLowerCase().endsWith(".pdf"));
  ...
  const numbered = links.slice(0, 2000).map((link, index) => {
    const parsed = new URL(link.url);
    const displayPath = parsed.search ? `${parsed.pathname}${parsed.search}` : parsed.pathname;
    const anchorDisplay = link.anchorText ? ` — ${link.anchorText}` : "";
    return `${index}. ${displayPath}${anchorDisplay}`;
  }).join("\n");
  const baseDomain = new URL(links[0].url).hostname;
  const prompt =
  ...
  try {
    const extraction = await geminiExtract<{ meeting_urls: number[] }>(prompt, MEETING_URL_SCHEMA);
    const seen = new Set<number>();
    const classified = (Array.isArray(extraction.meeting_urls) ? extraction.meeting_urls : [])
      .filter((idx): idx is number => Number.isInteger(idx) && idx >= 0 && idx < links.length)
      .filter((idx) => {
        if (seen.has(idx)) return false;
        seen.add(idx);
        return true;
      })
      .map((idx) => links[idx].url)
...
export function filterCivicDiscoveryCandidates<T extends { url: string }>(candidates: T[]): T[] {
...
function civicSortKey(url: string): [string, number] {
  ...
  let priority = 1;
  if (lower.includes("vollprotokoll") || lower.includes("wortprotokoll")) priority = 3;
  else if (
  ...
  if (lower.endsWith(".pdf")) priority += 10;
